...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    # scan_interval governs the settings poll. Clamp it: entries created when
    # this integration still had power sensors may carry a 30s value, which is
    # needlessly aggressive for settings that change rarely.
    scan_interval = max(
        MIN_SCAN_INTERVAL,
        int(
            entry.options.get(
    ...
    async def async_update_settings() -> dict[str, Any]:
        """Fetch the inverter settings object that backs the controls."""
        try:
            return await api.async_read_settings()
